...
from ..models import EnterpriseWechatAccessToken
...
class CorpApi(AbstractApi):
    def __init__(self, enterprise_wechat_app):
        self.app = enterprise_wechat_app
        self.corpid = self.app.corp_id
        self.secret = self.app.corp_secret
        self._token = None
    ...
    def getAccessToken(self):
        ...
        if self._token and self._token.is_expire():
            self.refreshAccessToken()
    ...
    def refreshAccessToken(self):
        # load token
        token = EnterpriseWechatAccessToken.objects.filter(
            enterprise_wechat_app=self.app).order_by("-created")[:1]
        if token and not token[0].is_expire():
            self._token = token[0]
        ...
        token = EnterpriseWechatAccessToken(access_token=response.get('access_token'),
                                            expire_in=response.get(
                                                "expires_in"),
                                            enterprise_wechat_app=self.app)
        token.save()
        self._token = token
